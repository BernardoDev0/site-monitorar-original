  ...
  useEffect(() => {
    const fetchInstagramPosts = async () => {
      try {
        ...
        console.log('🔥 TENTANDO PUXAR DA API REAL DO INSTAGRAM...');
        ...
        console.log('✅ DADOS RECEBIDOS DA API REAL:', data);
        ...
        if (data.data && data.data.length > 0) {
          ...
          console.log('✅ POSTS REAIS CARREGADOS:', imagePosts.length);
          setPosts(imagePosts);
        } else {
          throw new Error('Nenhum post encontrado na conta');
        }
      } catch (err) {
        console.error('❌ ERRO AO CARREGAR API REAL:', err);
      ...
      <section className="py-20 bg-gradient-to-br from-muted/20 to-muted/40">
        <div className="container mx-auto px-4">
          <div className="text-center">
            <Instagram className="h-12 w-12 mx-auto mb-4 text-red-500" />
            <h3 className="text-xl font-bold text-red-600 mb-4">🔥 TENTANDO PUXAR DA API REAL</h3>
            <p className="text-lg text-muted-foreground mb-4">Status da API do Instagram:</p>
            <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-4 text-left max-w-2xl mx-auto">
              <p className="text-sm text-red-700 font-mono">{error}</p>
            </div>
            <p className="text-sm text-muted-foreground">
              ✅ O código está CORRETO e tentando puxar da API real<br/>
